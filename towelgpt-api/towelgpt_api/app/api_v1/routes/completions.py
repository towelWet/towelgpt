...
def stream_completion(output: Iterable, base_response: CompletionStreamResponse):
    """
    Streams a towelgpt output to the client.

    Args:
        output: The output of towelgpt.generate(), which is an iterable of tokens.
...
async def completions(request: CompletionRequest):
    '''
    Completes a towelgpt model response.
    '''
    if settings.inference_mode == "gpu":
        ...
        if request.model != settings.model:
            raise HTTPException(status_code=400,
                                detail=f"The towelgpt inference server is booted to only infer: `{settings.model}`")
        ...
        model = towelgpt(model_name=settings.model, model_path=settings.towelgpt_path)
