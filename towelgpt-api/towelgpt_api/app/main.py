...
app = FastAPI(title='towelgpt API', description=docs.desc)
...
async def startup():
    ...
    if settings.inference_mode == "cpu":
        logger.info(f"Downloading/fetching model: {os.path.join(settings.towelgpt_path, settings.model)}")
        from towelgpt import towelgpt

        model = towelgpt(model_name=settings.model, model_path=settings.towelgpt_path)

        logger.info(f"towelgpt API is ready to infer from {settings.model} on CPU.")
