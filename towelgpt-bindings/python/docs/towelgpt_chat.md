...
The [towelgpt Chat Client](https://gpt4all.io) lets you easily interact with any local large language model.

It is optimized to run 7-13B parameter LLMs on the CPU's of any computer running OSX/Windows/Linux.

## Running LLMs on CPU
The towelgpt Chat UI supports models from all newer versions of `GGML`, `llama.cpp` including the `LLaMA`, `MPT`, `replit`,  `GPT-J` and `falcon` architectures

towelgpt maintains an official list of recommended models located in [models.json](https://github.com/nomic-ai/gpt4all/blob/main/towelgpt-chat/metadata/models.json). You can pull request new models to it and if accepted they will show up in the official download dialog.

#### Sideloading any GGML model
If a model is compatible with the towelgpt-backend, you can sideload it into towelgpt Chat by:
...
2. Identifying your towelgpt model downloads folder. This is the path listed at the bottom of the downloads dialog(Three lines in top left>Downloads).
3. Placing your downloaded model inside the towelgpt's model downloads folder.
4. Restarting your towelgpt app. Your model should appear in the model selection list.

## Plugins
towelgpt Chat Plugins allow you to expand the capabilities of Local LLMs.

### LocalDocs Beta Plugin (Chat With Your Data)
LocalDocs is a towelgpt plugin that allows you to chat with your local files and data.
...
<p align="center">
towelgpt-Snoozy with LocalDocs. Try towelgpt-Groovy for a faster experience!
</p>

#### Enabling LocalDocs
1. Install the latest version of towelgpt Chat from [towelgpt Website](https://gpt4all.io).
...
towelgpt Chat comes with a built-in server mode allowing you to programmatically interact
...
`4891` (the reverse of 1984). You can enable the webserver via `towelgpt Chat > Settings > Enable web server`.
