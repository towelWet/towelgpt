"""
Python only API for running all towelgpt models.
"""
import os
import time
...
# TODO: move to config
DEFAULT_MODEL_DIRECTORY = os.path.join(str(Path.home()), ".cache", "towelgpt").replace("\\", "\\\\")
...
class Embed4All:
    """
    Python class that handles embeddings for towelgpt.
    """
    ...
    def __init__(
        ...
        Args:
            n_threads: number of CPU threads used by towelgpt. Default is None, then the number of threads are determined automatically.
        """
        self.towelgpt = towelgpt(model_name='ggml-all-MiniLM-L6-v2-f16.bin', n_threads=n_threads)
    ...
    def embed(self, text: str) -> List[float]:
        ...
        """
        return self.towelgpt.model.generate_embedding(text)


class towelgpt:
    """
    Python class that handles instantiation, downloading, generation and chat with towelgpt models.
    """
    ...
    def __init__(
        ...
        Args:
            model_name: Name of towelgpt or custom model. Including ".bin" file extension is optional but encouraged.
            model_path: Path to directory containing model file or, if file does not exist, where to download model.
                Default is None, in which case models will be stored in `~/.cache/towelgpt/`.
    ...
    def retrieve_model(
        ...
        if allow_download:
            available_models = towelgpt.list_models()
        ...
        # If model file does not exist, download
            ...
            config["path"] = towelgpt.download_model(model_filename, model_path, verbose=verbose, url=url)
        else:
    ...
    def generate(
        ...
        """
        Generate outputs from any towelgpt model.
    ...
    def chat_session(
        ...
        """
        Context manager to hold an inference optimized chat session with a towelgpt model.
