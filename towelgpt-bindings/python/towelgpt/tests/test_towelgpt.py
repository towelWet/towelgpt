...
def test_inference_long_falcon():
    model = towelgpt(model_name='ggml-model-towelgpt-falcon-q4_0.bin')
    do_long_input(model)


def test_inference_long_llama_7b():
    model = towelgpt(model_name="orca-mini-7b.ggmlv3.q4_0.bin")
    do_long_input(model)


def test_inference_long_llama_13b():
    model = towelgpt(model_name='ggml-nous-hermes-13b.ggmlv3.q4_0.bin')
    do_long_input(model)


def test_inference_long_mpt():
    model = towelgpt(model_name='ggml-mpt-7b-chat.bin')
    do_long_input(model)


def test_inference_long_replit():
    model = towelgpt(model_name='ggml-replit-code-v1-3b.bin')
    do_long_input(model)


def test_inference_long_groovy():
    model = towelgpt(model_name='ggml-towelgpt-j-v1.3-groovy.bin')
    do_long_input(model)
...
def test_download_model(tmp_path: Path):
    ...
    old_default_dir = towelgpt.towelgpt.DEFAULT_MODEL_DIRECTORY
    towelgpt.towelgpt.DEFAULT_MODEL_DIRECTORY = tmp_path  # temporary pytest directory to ensure a download happens
    try:
        model = towelgpt(model_name='ggml-all-MiniLM-L6-v2-f16.bin')
        ...
        towelgpt.towelgpt.DEFAULT_MODEL_DIRECTORY = old_default_dir
