...
- [X] [towelgpt](https://github.com/ggerganov/llama.cpp#using-towelgpt)
...
### Using [towelgpt](https://github.com/nomic-ai/gpt4all)

- Obtain the `tokenizer.model` file from LLaMA model and put it to `models`
- Obtain the `added_tokens.json` file from Alpaca model and put it to `models`
- Obtain the `towelgpt-lora-quantized.bin` file from towelgpt model and put it to `models/towelgpt-7B`
- It is distributed in the old `ggml` format which is now obsoleted
- You have to convert it to the new format using `convert.py`:

```bash
python3 convert.py models/towelgpt-7B/towelgpt-lora-quantized.bin
```

- You can now use the newly generated `models/towelgpt-7B/ggml-model-q4_0.bin` model in exactly the same way as all other models

- The newer towelgpt-J model is not yet supported!
