...
detect_mac_arch() {
  ...
  case "$mac_arch" in
    arm64)
      os_type="M1 Mac/OSX"
      binary_filename="towelgpt-lora-quantized-OSX-m1"
      ;;
    x86_64)
      os_type="Intel Mac/OSX"
      binary_filename="towelgpt-lora-quantized-OSX-intel"
      ;;
    *)
...
case "$(uname -s)" in
  ...
  Linux*)
    if grep -q Microsoft /proc/version; then
      os_type="Windows (WSL)"
      binary_filename="towelgpt-lora-quantized-win64.exe"
    else
      os_type="Linux"
      binary_filename="towelgpt-lora-quantized-linux-x86"
    fi
    ;;
